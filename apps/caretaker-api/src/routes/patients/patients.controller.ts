...
import {PatientCreateFailedError, PatientNotFoundError} from "../../entities/patient/patient.errors";
...
export default class PatientsController {
    ...
    public static async createPatient(userId: number, patientDetails: Partial<PatientRecord>) {
        try {
            const user = await UserController.getById(userId);
            const patient = await PatientRepository.createPatient(Object.assign(patientDetails, {
                guardian: user
            }));
            return patient;
        } catch (error) {
    ...
    static async getById({userId, patientId}: { userId: number; patientId: number }): Promise<PatientRecord> {
        try {
    ...
    static async updatePatient({patientId, userId}: {
        userId: number;
        patientId: number
    }, patientDetails: Partial<PatientRecord>): Promise<PatientRecord> {
        try {
            const patient = await PatientRepository.findById(patientId, userId);
            if (!patient) {
                throw new PatientNotFoundError(patientId);
